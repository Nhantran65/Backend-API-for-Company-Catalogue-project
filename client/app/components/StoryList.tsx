...
import {stories, story_tags, tags} from '../constants/constants';
...
const StoryList: FC<StoryListProps> = () => {
	...
	story_tags.forEach((story_tag) => {
		...
		if (foundStoryIndex !== -1) {
			...
			if (name) {
				stories[foundStoryIndex].tag.push(name.tag_name.toLowerCase() as never);
			}
		}
	});
	...
	let filterStories = stories.filter(
		(story) =>
			listTag.length === 0 || listTag.every((t) => story.tag.includes(t as never))
	);
