...
const CompanyDetails = ({data: company, stories}: { data: any; stories: any[] }) => {
	...
	story_tags.forEach(story_tag => {
		...
		if (foundStoryIndex !== -1) {
			...
			const name: any = tags.find(tagName => story_tag.tag_id == tagName.tag_id);
			stories[foundStoryIndex].tag.push(name.tag_name.toLowerCase());
		}
	});

	const [listTag, setListTags] = useState<any>([]);
	const [listTagId, setTagId] = useState<any>([]);

	const onTagClick = (tag: any, tagId: any) => { //get tag id,
